...
float4 frag(vert2frag i) : COLOR {
	...
	for (int index = 0; index < 4; index++) {
		...
		float3 diffuseReflection = attenuation * unity_LightColor[index].rgb * max(0.0, dot(worldNormal, lightDirection));
		vertexLighting += diffuseReflection * 2;
	}

	lightColor += fixed4(vertexLighting, 1.0);
	lightColor += rimColor * _RimLightQuantity;
