	...
	SubShader
	{
		...
		Pass
		{
			...
			v2f vert (appdata v)
			{
                ...
                // Noise
                float noise1 = snoise(v.vertex + _Time);
                float noise2 = snoise(v.vertex + _Time * 2);
                float glitch = (noise1 - clamp(noise1, -0.7, 0.7)) *0.2 + noise2 * 0.004;
			...
			fixed4 frag (v2f i) : SV_Target
			{
				...
				// Scanlines
				float scan = 0.2;
                float scanMap = dirVertex * _ScanTiling + _Time.w * _ScanSpeed;
				scan += step(frac(scanMap), 0.1);
                scan += curve(frac(scanMap), -5) * 0.2;
